...
def is_local_maximum(acc, i, j, k, shape) -> bool:
    ...
    i_slice = np.s_[i-1:i+2] if i > 0 and i < shape[0] - 1 else (np.s_[i:i+2] if i == 0 else np.s_[i-1:])
    j_slice = np.s_[j-1:j+2] if j > 0 and j < shape[1] - 1 else (np.s_[j:j+2] if j == 0 else np.s_[j-1:])
    k_slice = np.s_[k-1:k+2] if k > 0 and k < shape[2] - 1 else (np.s_[k:k+2] if k == 0 else np.s_[k-1:])
...
def hough_circles(img, rows, cols, r_min, r_max, c_min, c_max, rad_min, rad_max) -> list:
    ...
    Returns:
        list            : List of N_circles detected circles 
    """
    acc = np.zeros((r_max - r_min + 1, c_max - c_min + 1, int(rad_max - rad_min) + 1), dtype=float)
                ...
                for r in range(r_min - 1, r_max):
                    for c in range (c_min - 1, c_max):
                        ...
                        if rad >= rad_min and rad < rad_max:
                            acc[r - r_min, c - c_max, rad - rad_min] += img[y, x]/rad
...
parser = argparse.ArgumentParser(description="Detect circles on images in /images folder")
parser.add_argument("--images", type=str, nargs='+', default=IMAGES, help="an image to detect circles")
parser.add_argument("-s", "--save", dest="save", action="store_true", default=False, help="save output images into /output folder (default: disabled)")
...
if __name__ == "__main__":
    ...
    for image_name in images:
        ...
        rad_min = RAD_MIN
        detected_circles = hough_circles(filtered_img, rows, cols, r_min, r_max, c_min, c_max, rad_min, rad_max)

        drawn_image = draw_circles(img, detected_circles, r_min, c_min, rad_min)
